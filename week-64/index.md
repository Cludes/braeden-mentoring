...
> Week 64. Cloud incident response.
> 
> When an attacker compromises a cloud environment, the response is different from a traditional network. You cannot just pull a network cable. You need to isolate resources, preserve evidence, revoke credentials, and analyse API call logs -- all without destroying evidence.
> 
> This week you work through a simulated cloud incident from detection to containment.
...
## Activity: Simulate and Respond to a Cloud Incident

1. Read the AWS Incident Response Guide: https://docs.aws.amazon.com/whitepapers/latest/aws-security-incident-response-guide/welcome.html -- take notes on the 5 phases
1. Simulate a credential compromise: create a low-privilege IAM user, then deliberately make several unusual API calls (list all S3 buckets, describe all EC2 instances)
1. In CloudTrail, find the unusual API calls you just made. Identify: what time, from what IP, which API calls
1. Practice isolation: disable the compromised IAM user's access keys, attach a deny-all policy to the user
1. Practice evidence preservation: create a CloudTrail snapshot, export the relevant events to S3
1. Write a 1-page incident report: timeline, what was accessed, what was done to contain, what should be done to prevent recurrence
1. You know you are done when you have a complete incident report covering detection, containment, and remediation
...
1. What is the first thing you should do when you suspect an AWS credential has been compromised?
2. Why is CloudTrail log integrity verification important during an incident?
3. What is the difference between isolation and remediation in incident response?
4. What AWS service would alert you in real time if an IAM credential was used from an unusual location?
