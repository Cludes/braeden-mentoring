...
> Week 65. Phase 12 capstone: build it all together.
> 
> You have covered SAST, secret scanning, container scanning, IaC scanning, CI/CD pipeline attacks, Vault, monitoring, and incident response. This week you build a single GitHub Actions pipeline that integrates every security check.
> 
> This is the kind of pipeline a professional DevSecOps engineer would build. By the end of this session you will have one to put in your portfolio.
...
## Activity: Full DevSecOps Pipeline

1. Create a new GitHub repo with a simple Python Flask app and a Dockerfile
1. Write a GitHub Actions workflow with these stages in order: (a) Bandit SAST scan, (b) detect-secrets scan, (c) pip-audit dependency check, (d) docker build, (e) Trivy image scan for CRITICAL CVEs, (f) Checkov IaC scan on any Terraform in the repo
1. Each stage must fail the pipeline on findings above a severity threshold you define
1. Write a SECURITY.md that documents: what each check does, what severity levels fail the build, and how to fix a failing check
1. Introduce a deliberate finding in each category. Confirm the pipeline catches all of them. Fix each one
1. Push the completed pipeline. Add a badge to your README showing the pipeline status
1. You know you are done when your pipeline has 6 security stages, all pass on clean code, and your SECURITY.md is written
...
1. Name the 6 security checks in your pipeline and what each one detects.
2. What is the difference between SAST and DAST?
3. Why should security checks be fast -- what happens to developer behaviour if a pipeline takes 20 minutes?
4. A developer pushes code with a hardcoded API key. Which stage catches it and what happens next?
