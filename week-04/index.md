...
> Week 4. This week: JavaScript.
> 
> HTMl and CSS build the page. JavaScript makes it do things. This is where coding actually starts.
> 
> Open your browser console right now (press F12, click Console). You can type JavaScript directly in there. Try: console.log(2 + 2). That is your playground for this week.
> 
> Note: JavaScript is case-sensitive. let and Let are different. This causes bugs. Pay attention to capitalisation.
...
1. Create a new file called guess.js
1. Generate a random integer between 1 and 100 using: Math.floor(Math.random() * 100) + 1
1. Use a loop that keeps running until the user guesses correctly
1. Use prompt() to get input from the user (prompt returns a string -- convert it to a number with parseInt())
1. After each guess, print 'Too high', 'Too low', or 'Correct! You took N guesses'
1. Run it in the browser console or with Node.js: node guess.js
1. You know you are done when the game correctly responds to high, low, and correct guesses and counts the number of attempts
...
1. What is the difference between let and const? When do you use each?
2. Why should you use === instead of ==?
3. What does a function return if it has no return statement?
4. What is the difference between an array and an object in JavaScript?
