...
> Week 32. HackTheBox Tier 2.
> 
> Tier 2 machines are unguided. There are no question prompts walking you through the steps. You get an IP address and a flag to find.
> 
> Recommended Tier 2 starting machines: Appointment (web, SQLi), Sequel (database misconfiguration), Crocodile (FTP + web), Responder (Windows credential capture).
> 
> Approach every machine with the same methodology: enumerate first, understand what is running, then look for a way in.
...
## Activity: HackTheBox Tier 2: Complete Two Machines

1. Pick any two Tier 2 machines from the Starting Point section
1. For each machine, follow the full methodology: start with nmap -sV, identify services, look for known vulnerabilities or misconfigurations
1. Write a complete write-up for each machine including: initial nmap output, how you found the vulnerability, what you did to exploit it, the user flag, the root flag
1. Save your write-ups to your ctf-practice/notes folder on GitHub
1. Do NOT look up write-ups until you have spent at least 45 minutes on a machine
1. You know you are done when you have two complete write-ups committed to GitHub
...
1. What is the first command you run after getting the IP address of a new machine?
2. Nmap shows ports 21 (FTP), 80 (HTTP), 443 (HTTPS) open. What do you check first and why?
3. You find a login page. What are the first three things you test?
4. What does 'getting a foothold' mean in a penetration test?
