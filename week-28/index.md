...
> Week 28. Regular expressions -- pattern matching for text.
> 
> Regex feels strange at first. After you get used to it, it becomes one of the most powerful tools you have. Security professionals use regex to extract IPs from logs, find credentials in code, parse HTTP responses, and filter output from tools.
> 
> Practice regex interactively at: https://regex101.com -- paste a pattern and test text in real time.
...
## Activity: Credential Scanner

1. Write a Python script that scans a directory of files for common sensitive patterns
1. Look for: email addresses, IP addresses, URLs (http:// and https://), private keys (-----BEGIN ... PRIVATE KEY-----), AWS access key patterns (AKIA followed by 16 uppercase alphanumeric characters), password= or api_key= followed by a value
1. For each match: print the filename, line number, and the matched text (truncate to 80 chars)
1. The script should accept a directory path as a command-line argument: python scanner.py ./test_files
1. Create 3 test files with intentionally embedded fake credentials to verify your patterns work
1. You know you are done when it finds all the planted credentials and reports them with accurate file and line numbers
...
2. What does \b in a regex pattern do?
3. What is a capture group and how do you access it after a match?
4. What does the ? make a quantifier do (as in .*? vs .*)? When would you use the lazy version?
