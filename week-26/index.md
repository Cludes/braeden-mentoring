...
> Week 26. Phase 6: Python automation.
> 
> You know Python basics. Now you will use it to do real work: read and process files, parse logs, automate tasks.
> 
> Security professionals write Python scripts constantly -- to process scan results, parse logs, automate report generation, and interact with APIs. This week covers the foundation of all of that.
...
1. Open the activity at activities/random-it/short-exercises/03_log-parsing.md
1. Write a Python script that reads a web server access log (Apache/Nginx format)
1. Parse each line to extract: IP address, HTTP method, URL path, status code, response size
1. Generate a report showing: top 10 IP addresses by request count, top 10 URLs by request count, count of each status code (200, 404, 500, etc.), total requests and total data transferred
1. Save the report to report.txt
1. You know you are done when the report accurately reflects the log file data and is readable by a non-technical person
...
1. What is the difference between r (read), w (write), and a (append) modes when opening a file?
2. Why should you always use with open(...) instead of f = open(...)?
3. What does strip() do and why do you almost always call it when reading lines from a file?
4. How would you process a 10GB log file without loading it all into memory at once?
