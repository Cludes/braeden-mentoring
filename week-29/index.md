...
> Week 29. Using Python to talk to APIs.
> 
> You have already used fetch() in JavaScript. The Python equivalent is the requests library. The concepts are the same: send a request, get a response, parse the JSON.
> 
> This week you will use the GitHub API to do something useful: automate a task that would normally require clicking through a web interface.
...
## Activity: GitHub Repo Analyser

1. Install requests: pip install requests
1. Write a Python script that takes a GitHub username as input and retrieves all their public repos via the GitHub API: https://api.github.com/users/{username}/repos
1. For each repo, display: name, description, primary language, star count, last updated date
1. Sort repos by star count (highest first)
1. Calculate and display: total repos, total stars, most used language
1. Add rate limit handling -- check the X-RateLimit-Remaining header and warn if below 10
1. You know you are done when the output for a real GitHub user matches what you see on their profile page
...
2. How do you pass query parameters in a requests.get() call?
3. The GitHub API requires authentication for more than 60 requests per hour. How would you add a personal access token to your requests?
4. What is pagination in an API and how would you handle a case where there are more results than one page can return?
